<?php
/*
    $data = $menuel['elements']
*/

$appMenus = [
    'sidebar menu' => [
        [
            'slug' => 'link',
            'href' => '/home',
            'name' => 'Dashboard',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-speedometer'
        ],
        [
            'slug' => 'title',
            'name' => 'Management',
            'hasIcon' => false,
            'iconType' => 'coreui',
            'icon' => ''
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Kelola User',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/user',
                    'name' => 'Daftar User',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/dosen',
                    'name' => 'Daftar Dosen',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/dekan',
                    'name' => 'Kelola Dekan',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/kajur',
                    'name' => 'Kelola Ketua Jurusan',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/auditor',
                    'name' => 'Kelola Auditor',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
            ]
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Kelola Instrumen Audit',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/standard',
                    'name' => 'Standar AMI',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
            ]
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Kelola Periode AMI',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/periode',
                    'name' => 'Periode AMI',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/instrument',
                    'name' => 'Periode Standard AMI',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
            ]
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Fakultas',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/faculties',
                    'name' => 'Daftar Fakultas',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
            ]
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Audit',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/prodi-audit',
                    'name' => 'Daftar Audit Prodi',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/prodi-auditor',
                    'name' => 'Kelola Penempatan Auditor',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],

            ]
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Auditee view',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/skor-taksiran-audit',
                    'name' => 'Isi Skor Taksiran Audit',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/cetak-berita-acara',
                    'name' => 'Cetak Berita Acara',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/kelola-tindakan-koreksi',
                    'name' => 'Kelola Tindakan Koreksi',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],


            ]
        ],
        [
            'slug' => 'dropdown',
            'name' => 'Auditor view',
            'hasIcon' => true,
            'iconType' => 'coreui',
            'icon' => 'cil-address-book',
            'elements' => [
                [
                    'slug' => 'link',
                    'href' => '/skor-audit',
                    'name' => 'Isi Skor Audit',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/lihat-berita-acara',
                    'name' => 'Lihat Berita Acara',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/isi-temuan-audit',
                    'name' => 'Isi Temuan Audit',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],
                [
                    'slug' => 'link',
                    'href' => '/kelola-tindakan-koreksi',
                    'name' => 'Kelola Tindakan Koreksi',
                    'hasIcon' => true,
                    'iconType' => 'coreui',
                    'icon' => 'cil-apps'
                ],

            ]
        ],
    ]
];

if(!function_exists('renderDropdown')){
    function renderDropdown($data){
        if(array_key_exists('slug', $data) && $data['slug'] === 'dropdown'){
            echo '<li class="c-sidebar-nav-dropdown">';
            echo '<a class="c-sidebar-nav-dropdown-toggle" href="#">';
            if($data['hasIcon'] === true && $data['iconType'] === 'coreui'){
                echo '<i class="' . $data['icon'] . ' c-sidebar-nav-icon"></i>';
            }
            echo $data['name'] . '</a>';
            echo '<ul class="c-sidebar-nav-dropdown-items">';
            renderDropdown( $data['elements'] );
            echo '</ul></li>';
        }else{
            for($i = 0; $i < count($data); $i++){
                if( $data[$i]['slug'] === 'link' ){
                    echo '<li class="c-sidebar-nav-item">';
                    echo '<a class="c-sidebar-nav-link" href="' . $data[$i]['href'] . '">';
                    echo '<span class="c-sidebar-nav-icon"></span>' . $data[$i]['name'] . '</a></li>';
                }elseif( $data[$i]['slug'] === 'dropdown' ){
                    renderDropdown( $data[$i] );
                }
            }
        }
    }
}
?>

      <div class="c-sidebar-brand"><img class="c-sidebar-brand-full" src="{{ asset('/assets/brand/coreui-base-white.svg') }}"
      width="118" height="46" alt="CoreUI Logo"><img class="c-sidebar-brand-minimized" src="assets/brand/coreui-signet-white.svg"
      width="118" height="46" alt="CoreUI Logo"></div>
        <ul class="c-sidebar-nav">
        @if(isset($appMenus['sidebar menu']))
            @foreach($appMenus['sidebar menu'] as $menuel)
                @if($menuel['slug'] === 'link')
                    <li class="c-sidebar-nav-item">
                        <a class="c-sidebar-nav-link" href="{{ env('APP_URL', '') . $menuel['href'] }}">
                        @if($menuel['hasIcon'] === true)
                            @if($menuel['iconType'] === 'coreui')
                                <i class="{{ $menuel['icon'] }} c-sidebar-nav-icon"></i>
                            @endif
                        @endif
                        {{ $menuel['name'] }}
                        </a>
                    </li>
                @elseif($menuel['slug'] === 'dropdown')
                    <?php renderDropdown($menuel) ?>
                @elseif($menuel['slug'] === 'title')
                    <li class="c-sidebar-nav-title">
                        @if($menuel['hasIcon'] === true)
                            @if($menuel['iconType'] === 'coreui')
                                <i class="{{ $menuel['icon'] }} c-sidebar-nav-icon"></i>
                            @endif
                        @endif
                        {{ $menuel['name'] }}
                    </li>
                @endif
            @endforeach
        @endif
        </ul>
        <button class="c-sidebar-minimizer c-class-toggler" type="button" data-target="_parent"
        data-class="c-sidebar-minimized"></button>
    </div>